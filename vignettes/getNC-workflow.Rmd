...
if (requireNamespace("plotly", quietly = TRUE)) {
  ...
  cat("plotly not installed; skipping 3D plots.\n")
}
```
...
```{r}
y_shift <- rnorm(40, mean = mu_c + 1, sd = sd_c)
validate_knockout_group_from_fit(fit, target, knocked, y_shift)
```

### Variance inflation

```{r}
y_inflate <- rnorm(40, mean = mu_c, sd = sd_c * 2)
validate_knockout_group_from_fit(fit, target, knocked, y_inflate)
```
